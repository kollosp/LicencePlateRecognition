...
def main():
    ...
    cv.createTrackbar("Canny max",window_name, 0, 255, canny_max)
    cv.createTrackbar("Canny min",window_name, 0, 255, canny_min)
    ...
    cv.setTrackbarPos("Canny max", window_name, canny[0])
    cv.setTrackbarPos("Canny min", window_name, canny[1])

    while True:

        frame_hsv = cv.cvtColor(src, cv.COLOR_BGR2HSV)
        frame_threshold = cv.inRange(frame_hsv, color_min, color_max)
        canny_img = cv.Canny(frame_threshold, canny[0], canny[1])

        canny_img = cv.cvtColor(canny_img, cv.COLOR_GRAY2RGB)
        frame_threshold = cv.cvtColor(frame_threshold, cv.COLOR_GRAY2RGB)
        img = concat_tile_resize([[src, frame_threshold, canny_img]])
